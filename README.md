## Requirements

| Name | Version |
|------|---------|
| <a name="requirement_google"></a> [google](#requirement\_google) | 3.63.0 |

## Providers

| Name | Version |
|------|---------|
| <a name="provider_google"></a> [google](#provider\_google) | 3.63.0 |

## Modules

| Name | Source | Version |
|------|--------|---------|
| <a name="module_chain"></a> [chain](#module\_chain) | ./modules/chain |  |
| <a name="module_jenkins"></a> [jenkins](#module\_jenkins) | ./modules/jenkins |  |
| <a name="module_storage"></a> [storage](#module\_storage) | ./modules/storage |  |
| <a name="module_vpc"></a> [vpc](#module\_vpc) | ./modules/vpc |  |

## Resources

| Name | Type |
|------|------|
| [google_compute_project_metadata.default_project_metadata](https://registry.terraform.io/providers/hashicorp/google/3.63.0/docs/resources/compute_project_metadata) | resource |

## Inputs

| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_GOOGLE_BACKEND_CREDENTIALS"></a> [GOOGLE\_BACKEND\_CREDENTIALS](#input\_GOOGLE\_BACKEND\_CREDENTIALS) | Google credentaisl | `string` | n/a | yes |
| <a name="input_chain_tag_version"></a> [chain\_tag\_version](#input\_chain\_tag\_version) | Version of nodle chain to deploy | `string` | `"sub3"` | no |
| <a name="input_project_id"></a> [project\_id](#input\_project\_id) | id of the project | `string` | `"nodle-chain-d08ab"` | no |
| <a name="input_public_ssh_keys_map"></a> [public\_ssh\_keys\_map](#input\_public\_ssh\_keys\_map) | SSH Keys with user for the VMs | `map` | <pre>{<br>  "nofal": "ssh-rsa  ahmad@ahmad-working-laptop"<br>}</pre> | no |
| <a name="input_region"></a> [region](#input\_region) | region of GCP to deploy the project | `string` | n/a | yes |
| <a name="input_zone"></a> [zone](#input\_zone) | zone of GCP to deploy the project | `string` | n/a | yes |

## Outputs

No outputs.


## Debug

To debug gcp startup script

``` sh
sudo google_metadata_script_runner startup
```
